...
export default {
  ...
  computed: {
    ...mapState({
      ...
      currRouterId: (state) => state.user.currRouterId
    })
  },
  watch: {
    sidebarOpened(val) {
    ...
    currRouterId(val) {
      this.menus = this.fetchChild(this.permissionMenuList, val)
      console.log('---------------------当前左侧菜单路由---------------------')
  ...
  created() {
    ...
    this.menus = this.fetchChild(this.permissionMenuList)
  ...
  methods: {
    ...
    fetchChild(data, id = 'd7d6e2e4e2934f2c9385a623fd98c6f3') {
      let newData = []
      data.map((item, idx) => {
        if (item.id == id) {
