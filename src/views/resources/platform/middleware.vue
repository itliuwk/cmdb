  ...
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form-model :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-row>
          <a-col :span="6">
            <a-form-model-item label="CLUSTER名称">
              <a-input placeholder />
            </a-form-model-item>
          </a-col>
          <a-col :span="6">
            <a-form-model-item label="集群类型">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :span="6">
              <a-form-model-item label="MDM物理实例">
                <a-select placeholder="请选择" default-value="0">
                  <a-select-option value="0">全部</a-select-option>
                  <a-select-option value="1">关闭</a-select-option>
                  <a-select-option value="2">运行中</a-select-option>
    ...
    <a-modal :title="title" width="800px" :visible="visible" @ok="handleOk" @cancel="handleCancel">
      <d-middleware ref="dMiddleware"></d-middleware>
    </a-modal>
  </a-card>
</template>
...
const columns = [
  ...
  { title: '集群名称', width: 100, dataIndex: 'type', key: 'name' },
  { title: '集群类型', dataIndex: 'cluster', key: '1' },
  { title: '中间件版本', dataIndex: 'version', key: '2' },
  { title: '集群IP', dataIndex: 'ip', key: '3' },
  // { title: 'MDM物理实例', dataIndex: 'physicalInstance', key: '4' },
  { title: '所属集群', dataIndex: 'type', key: '5' },
  { title: '所属虚拟机', dataIndex: 'virtual', key: '6' },
  { title: '所属物理机', dataIndex: 'physical', key: '7' },
  {
    title: '操作',
    key: 'operation',
    width: 200,
...
import dMiddleware from './dialog/d-middleware.vue'
...
export default {
  ...
  methods: {
    ...
    handleOk() {
      ...
      let dMiddleware = this.$refs['dMiddleware']
      dMiddleware.onSubmit().then((res) => {
        if (res.is) {
